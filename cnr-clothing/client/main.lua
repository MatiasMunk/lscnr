...
RegisterNetEvent('CNRCore:Client:OnPlayerLoaded')
AddEventHandler('CNRCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("cnr-clothes:loadPlayerSkin")
    PlayerData = CNRCore.Functions.GetPlayerData()
end)

RegisterNetEvent('CNRCore:Client:OnJobUpdate')
AddEventHandler('CNRCore:Client:OnJobUpdate', function(JobInfo)
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    gender = "male"
                                    if CNRCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
                                    CNRCore.Functions.TriggerCallback('cnr-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNetEvent('cnr-clothing:client:openOutfitMenu')
AddEventHandler('cnr-clothing:client:openOutfitMenu', function()
    CNRCore.Functions.TriggerCallback('cnr-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('cnr-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('cnr-clothing:client:openMenu')
AddEventHandler('cnr-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
...
function openMenu(allowedMenus)
    ...
    local PlayerData = CNRCore.Functions.GetPlayerData()
...
RegisterNUICallback('TrackerError', function()
    CNRCore.Functions.Notify("You can't remove your ankle bracelet ..", "error")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('cnr-clothes:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('cnr-clothing:client:reloadOutfits')
AddEventHandler('cnr-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('cnr-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    CNRCore.Functions.Notify("You have deleted your"..data.outfitName.." outfit!")
end)
...
function SaveSkin()
	...
	TriggerServerEvent("cnr-clothing:saveSkin", model, clothing)
end

RegisterNetEvent('cnr-clothes:client:CreateFirstCharacter')
AddEventHandler('cnr-clothes:client:CreateFirstCharacter', function()
    CNRCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent("cnr-clothes:loadSkin")
AddEventHandler("cnr-clothes:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('cnr-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('cnr-clothing:client:loadPlayerClothing')
AddEventHandler('cnr-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('cnr-clothing:client:loadOutfit')
AddEventHandler('cnr-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if CNRCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        CNRCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
...
RegisterNetEvent("cnr-clothing:client:adjustfacewear")
AddEventHandler("cnr-clothing:client:adjustfacewear",function(type)
    if CNRCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
function reloadSkin(health)
    ...
    local gender = CNRCore.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("cnr-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("cnr-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
